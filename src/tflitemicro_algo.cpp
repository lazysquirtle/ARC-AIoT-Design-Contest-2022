...
extern "C" int tflitemicro_algo_run(float *ptr_data)
{
	int ercode = 0;
	for (int i = 0; i < kSize; i++){
		input->data.f[i] = *(ptr_data + i);
		sprintf(uart_buf, "input [%d] : %f \r\n", i, input->data.f[i]);
	...
	{
		//error_reporter->Report("[%d]: %f", i, results_ptr[i]);
		if (i == 0)
			sprintf(uart_buf, "[N]: %f\r\n",results_ptr[i]);
		else
			sprintf(uart_buf, "[C]: %f\r\n",results_ptr[i]);
		uart0_ptr->uart_write(uart_buf, strlen(uart_buf));
		board_delay_ms(20);

		if ((results_ptr[i] > 0.0) && (maxScore < results_ptr[i]))
		{
